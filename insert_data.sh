...
# Read from the CSV file, skipping the header line
tail -n +2 games.csv | while IFS="," read YEAR ROUND WINNER OPPONENT WIN_GOALS OPP_GOALS
do
  # Insert the winner into the teams table if not already present
  ...
  # Insert the opponent into the teams table if not already present
